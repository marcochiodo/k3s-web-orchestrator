...
registry_status() {
    ...
    local domains=$(echo "$config" | jq -r '.domains | join(", ")')
    ...
    echo "  Domain: $domain"
    [ "$domains" != "$domain" ] && echo "  All domains: $domains"
    ...
    if [ "$pod_ready" = "true" ]; then
        ...
        local d response
        for d in $(echo "$config" | jq -r '.domains[]'); do
            response=$(curl -s -o /dev/null -w "%{http_code}" \
                -u "$cred_username:$cred_password" \
                "https://$d/v2/" 2>/dev/null || echo "000")

            if [ "$response" = "200" ]; then
                echo "  ✓ $d accessible (HTTP $response)"
            else
                echo "  ✗ $d not accessible (HTTP $response)"
...
registry_rotate_credentials() {
    ...
    # Update registries.yaml (one entry per domain)
    log_info "Updating /etc/rancher/k3s/registries.yaml..."
    write_registries_yaml "$username" "$new_password" $(echo "$config" | jq -r '.domains[]')
